...
def get_strings_from_bin(filename, minlen=4):
    ...
    Note: Use get_raw_strings_from_bin for better precision
    """
...
def get_raw_strings_from_bin(filename,
                             min=4,
                             start_offset=0,
                             end_offset=0):
...
def generate_strings(filename, start_offset=0, end_offset=0, minlen=4):
    ...
    try:
        ...
        for s, str_offset in get_raw_strings_from_bin(filename,
                                                      minlen,
                                                      start_offset,
                                                      end_offset):
...
def search_string_in_file(filename,
                          search_string,
                          start_offset=0,
                          end_offset=0,
                          minlen=4):
